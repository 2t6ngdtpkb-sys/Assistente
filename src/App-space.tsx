            ...
            {messages.map((m, i) => (
              <div key={i} style={{ display: "flex", gap: 12, animation: "fadeIn .25s ease" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 14.5, lineHeight: 1.7, color: "#27282D", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{m.content}</div>
                </div>
              </div>
            ))}
            {loading && (
          ...
          <div style={{ maxWidth: 720, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: 8, border: `1px solid ${hairline}`, borderRadius: 16, padding: "6px 6px 6px 16px", background: surfaceMute }}>
              ...
              <button onClick={sendMessage} disabled={loading || !input.trim()}
                style={{ width: 38, height: 38, borderRadius: 11, flexShrink: 0, border: "none", cursor: loading || !input.trim() ? "default" : "pointer",
                  background: loading || !input.trim() ? "#E2E3E7" : ink, color: "#fff", fontSize: 14, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>↑</button>
            </div>
            <div style={{ textAlign: "center", fontSize: 11, color: inkSoft, marginTop: 8 }}>
              {deepResearch ? `${ui.advanced} · ${ui.deepResearch}` : isPro ? `${ui.advanced} · ${ui.advancedDesc}` : "AstraAI"}
            </div>
          </div>
        </div>
      </main>
